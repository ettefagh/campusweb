<script lang="ts">
	...
	import MonthCalendarWidget from "$lib/components/MonthCalendarWidget.svelte";
	...
	function sortableFavoriteContacts(
		node: HTMLElement,
		options: { enabled: boolean },
	) {
		let sortable: Sortable | null = null;
		...
		function applyMasonry() {
			...
			items.forEach((item) => {
				...
				for (let i = 0; i < item.children.length; i++) {
					const rect = item.children[i].getBoundingClientRect();
					if (rect.height > 0) {
	...
	let calendarWidgetMode: "today" | "next" | "month" = "today";
	...
	$: calendarWidgetTitle =
		calendarWidgetMode === "next"
			? $t.home.nextEvents
			: getTodayCalendarTitle();
	...
	function updateUpcomingEvents() {
		...
		const subscriptionEvents = calendarSubscriptions.flatMap(
			(subscription) => subscription.cachedEvents,
		);
		const allEvents = [
			...staticCalendarEvents,
			...subscriptionEvents,
	...
	function formatEventEndTime(event: CalendarEvent) {
		...
		return event.end.toLocaleTimeString([], {
			hour: "2-digit",
			minute: "2-digit",
		});
	}
	...
	function resolveEventDotColor(event: CalendarEvent) {
		...
		if (classGroupId) {
			...
			const classDef = $activeClasses.find(
				(cls) => cls.id === classGroupId,
			);
	...
	$: displayLinks = isEditMode
		? allLinks.filter((link) => {
				...
				const title =
					$t.linkTitle?.[link.id as keyof typeof $t.linkTitle] ||
					link.title;
				const desc =
					$t.linkDesc?.[link.id as keyof typeof $t.linkDesc] ||
					link.description ||
					"";
				return (
	...
	$: if (activeHomeBlocks.length > 0) {
		...
		browser &&
		!isArrangingHomeBlocks &&
		!hasAutoOpenedAvailableBlocks
	) {
		isArrangingHomeBlocks = true;
	...
	$: displayFavoriteContacts = $favoriteContacts.map((email) => {
		...
		if (normalized === "default:course-coordinator") {
			const cc = [...homeCampusContacts, ...homeGeneralContacts].find(
				(c) => c.service === "Course Coordination",
			);
			if (cc) {
				return {
					...cc,
					services: ["Course Coordination"],
					programs: [],
					tags: cc.tags || [],
				};
			}
			return {
				...
				tags: [],
				locked:
					!$settingsStore.emailVerified || !$settingsStore.campusId,
			};
		}
		...
		if (normalized === "default:study-advisor") {
			...
			if (advisor) {
				return {
					...
					services: [],
					programs: [
						`${advisor.degree || ""} ${advisor.program || ""}`.trim(),
					].filter(Boolean),
					tags: advisor.tags || [],
				};
			}
	...
	function getHomeSectionLabel(id: string) {
		...
		if (id === "favorites")
			return $t.home.universityLinks || $t.settings.favoriteLinks;
	...
	function reorderHomeBlocks(orderedBlockIds: string[]) {
		const sectionsById = new Map(
			$settingsStore.homeSections.map((section) => [section.id, section]),
		);
		const header = sectionsById.get("header") ?? {
			id: "header",
			enabled: true,
		};
		...
		const remainingSections = $settingsStore.homeSections.filter(
			(section) =>
				section.id !== "header" &&
				!orderedBlockIds.includes(section.id),
		);

		settingsStore.patch({
			homeSections: [header, ...orderedBlocks, ...remainingSections],
		});
	}
	...
	function setHomeBlockEnabled(id: string, enabled: boolean) {
		...
		const activeBlocks = $settingsStore.homeSections.filter(
			(section) =>
				section.id !== "header" && section.id !== id && section.enabled,
		);
	...
	async function loadHomePrivateContacts(isVerified: boolean) {
		...
		if (loadedHomeContactsAuthState === true || isHomeContactsLoading)
			return;
	...
	function getSchoolFromTags(tags?: string[]) {
		const found = tags?.find((tag) =>
			tag.toLowerCase().startsWith("school:"),
		);
	...
	function buildHomeContactDirectory(
		...
		const allContacts = [
			...
			...programDirectors.map((contact) => ({
				...
				services: [],
				programs: [
					`${contact.degree || ""} ${contact.program || ""}`.trim(),
				].filter(Boolean),
				school: contact.school?.toLowerCase(),
				tags: contact.degree
					? [contact.degree, ...(contact.tags || [])]
					: contact.tags || [],
			})),
		];
		...
		for (const contact of allContacts) {
			...
			if (existing) {
				for (const service of contact.services || []) {
					if (service && !existing.services.includes(service))
...
<div class="home-page" style:display={active ? "flex" : "none"}>
	...
	{#each $settingsStore.homeSections as section, i (section.id)}
		{#if section.enabled && section.id === "header"}
			<header
				class="home-hero"
				class:compact={$settingsStore.headerSize === "small"}
			>
	...
	<div
		...
		use:sortableHomeBlocks={{ enabled: isArrangingHomeBlocks }}
		use:masonryGrid
	>
		{#each $settingsStore.homeSections as section, i (section.id)}
			{#if section.enabled && section.id !== "header"}
				<div
					class="home-block"
					class:home-block--full={blockLayoutConfigs[section.id]
						?.fullWidth}
					class:home-block--span-2={!blockLayoutConfigs[section.id]
						?.fullWidth &&
						blockLayoutConfigs[section.id]?.minCols === 2}
					class:home-block--span-3={!blockLayoutConfigs[section.id]
						?.fullWidth &&
						blockLayoutConfigs[section.id]?.minCols === 3}
					class:home-block--left={blockLayoutConfigs[section.id]
						?.position === "left"}
					class:home-block--right={blockLayoutConfigs[section.id]
						?.position === "right"}
					class:is-arranging={isArrangingHomeBlocks}
					data-section-id={section.id}
					style={i <= 1
						? ""
						: `animation: reveal 0.6s cubic-bezier(0.22, 1, 0.36, 1) backwards; animation-delay: ${i * 100}ms;`}
				>
					...
					{:else if section.id === "favorites"}
						...
						>
							<SectionHeader
								title={$t.home.universityLinks ||
									$t.settings.favoriteLinks}
								subtitle={$t.home.customBookmarks}
							>
							...
							{:else}
								{#each displayLinks as link (link.id)}
									<LinkCard
										...
										editMode={isEditMode}
										reorderMode={isManagingFavorites &&
											!isEditMode &&
											!isArrangingHomeBlocks}
					...
					{:else if section.id === "favoriteContacts"}
						<section
							...
							use:sortableFavoriteContacts={{
								enabled:
									isManagingFavoriteContacts &&
									!isArrangingHomeBlocks,
							}}
						>
							<SectionHeader
									...
									{#each displayFavoriteContacts as contact (contact.email)}
										...
										>
											...
											{#if isManagingFavoriteContacts}
												...
												>
													<i
														class="ph-bold ph-minus-circle"
														aria-hidden="true"
													></i>
												</div>
											{:else if contact.locked}
												<div
													class="favorite-contact-action locked-indicator"
													aria-hidden="true"
												>
													<i
														class="ph-bold ph-lock-key"
						...
						<section class="links-section" id="calendar">
							...
							<div class="calendar-preview">
								{#if isLoadingCalendarPreview}
									<p class="calendar-preview-empty">
										{$t.calendar.loading ||
											"Loading calendar..."}
									</p>
								{:else if calendarWidgetMode === "month"}
									<MonthCalendarWidget
										events={[
											...staticCalendarEvents,
											...calendarSubscriptions.flatMap(
												(s) => s.cachedEvents,
											),
											...$holidayEvents,
										]}
									...
									<ul class="calendar-preview-list">
										{#each upcomingEvents as event (event.id)}
											<li class="calendar-preview-item">
												...
												>
													<div
														class="calendar-preview-dot"
														style={`background:${resolveEventDotColor(event)}`}
													></div>
													{#if ($settingsStore.calendarWidgetMode || "today") === "today"}
														<div
															class="calendar-preview-content calendar-preview-content-today"
														>
															<div
																class="calendar-preview-main"
															>
																<p
																	class="calendar-preview-title"
																>
																	{event.title}
																</p>
																<p
					...
					{#if isArrangingHomeBlocks}
						<div class="home-block-controls">
								...
								>
									<select
										aria-label={$t.settings
											.calendarWidgetMode}
										value={$settingsStore.calendarWidgetMode ||
											"today"}
										on:change={(event) =>
											settingsStore.patch({
												calendarWidgetMode: (
									...
									>
										<option value="today"
											>{$t.settings
												.calendarWidgetToday}</option
										>
										<option value="next"
											>{$t.settings
												.calendarWidgetNext}</option
										>
							...
							<button
								...
								on:click={() =>
									setHomeBlockEnabled(section.id, false)}
							>
								<i
									class="ph-bold ph-minus-circle"
									aria-hidden="true"
	...
	{#if isArrangingHomeBlocks}
		<div class="available-blocks-wrapper">
			<section
				class="available-blocks-panel"
				aria-labelledby="available-home-blocks-title"
			>
				<div class="available-blocks-header">
					<span class="available-blocks-kicker"
						>{$t.home.availableBlocksKicker}</span
					>
					<h2 id="available-home-blocks-title">
						{$t.home.availableBlocks}
					</h2>
					<p>{$t.home.availableBlocksDesc}</p>
				</div>

				{#if availableHomeBlocks.length === 0}
					<p class="available-blocks-empty">
						{$t.home.noAvailableBlocks}
					</p>
				{:else}
	...
	<div class="fab-group">
		...
		<button
			...
			aria-label={isArrangingHomeBlocks
				? $t.home.doneArrangeBlocks
				: $t.home.arrangeBlocks}
			title={isArrangingHomeBlocks
				? $t.home.doneArrangeBlocks
				: $t.home.arrangeBlocks}
		>
			{#if isArrangingHomeBlocks}
		...
		>
			...
			<div class="home-event-detail-list">
				<p>
					...
					<strong
						>{formatEventFullDate(selectedCalendarEvent)} · {formatEventTime(
				...
				{#if selectedCalendarEvent.extendedProps?.location || selectedCalendarEvent.extendedProps?.shortLocation || selectedCalendarEvent.extendedProps?.locationUrl}
					<p>
						...
						{#if selectedCalendarEvent.extendedProps?.locationUrl}
							<a
								href={selectedCalendarEvent.extendedProps
									.locationUrl}
								target="_blank"
						...
						{:else if getLocationHref(selectedCalendarEvent)}
							<a
								href={getLocationHref(selectedCalendarEvent)}
								target="_blank"
								rel="noopener noreferrer"
							>
								{selectedCalendarEvent.extendedProps
